...
def process_single_image(sample_file, label_file, pool):
    _, sample_num = load_label(label_file)  # We assume label_file has only one label
    data = load_sample(sample_file, sample_num, pool)
    processed_data = data[0].flatten()  # Process only the first image
    label = np.array(load_label(label_file)[0][0])  # Load only the first label
...
def main():
    ...
    w, b = model(x_train, y_train)  # Train model with all training data

    # Now test on a single image
    single_image, actual_label = process_single_image(test, test_label, pool)
    single_image = single_image.reshape(1, -1)  # Reshape for compatibility with prediction function
    predicted_label = predict(w, b, single_image)
